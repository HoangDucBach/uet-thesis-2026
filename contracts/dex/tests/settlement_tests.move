...
const EOUTPUT_INSUFFICIENT: u64 = 2;
const ENOT_WINNER: u64 = 4;
const ESCORE_MISMATCH: u64 = 5;
const EINVALID_DEADLINE: u64 = 6;
const EWRONG_BATCH: u64 = 7;
...
#[test]
fun test_process_intent_increments_cow_pairs() {
    let mut ctx = tx_context::dummy();

    let mut ticket = settlement::create_ticket_for_testing(42, 1);
    let intent = intent_book::create_intent_for_testing<SUI, USDC>(100, 50, 9999, 42, &mut ctx);
    let payout = coin::mint_for_testing<USDC>(60, &mut ctx);

    settlement::process_intent<SUI, USDC>(&mut ticket, intent, payout, 50, 55, &mut ctx);

    assert!(settlement::ticket_actual_cow_pairs(&ticket) == 1);

    settlement::destroy_ticket_for_testing(ticket);
}

#[test]
#[expected_failure(abort_code = EOUTPUT_INSUFFICIENT, location = cow_dex::settlement)]
fun test_process_intent_payout_below_min_required_aborts() {
    let mut ctx = tx_context::dummy();

    let mut ticket = settlement::create_ticket_for_testing(42, 1);
    let intent = intent_book::create_intent_for_testing<SUI, USDC>(100, 50, 9999, 42, &mut ctx);
    let payout = coin::mint_for_testing<USDC>(40, &mut ctx);

    settlement::process_intent<SUI, USDC>(&mut ticket, intent, payout, 50, 30, &mut ctx);

    settlement::destroy_ticket_for_testing(ticket);
}

#[test]
#[expected_failure(abort_code = 3, location = cow_dex::settlement)]
fun test_process_intent_payout_below_ebbo_floor_aborts() {
    let mut ctx = tx_context::dummy();

    let mut ticket = settlement::create_ticket_for_testing(42, 1);
    let intent = intent_book::create_intent_for_testing<SUI, USDC>(100, 50, 9999, 42, &mut ctx);
    let payout = coin::mint_for_testing<USDC>(55, &mut ctx);

    settlement::process_intent<SUI, USDC>(&mut ticket, intent, payout, 50, 60, &mut ctx);

    settlement::destroy_ticket_for_testing(ticket);
}

#[test]
#[expected_failure(abort_code = EWRONG_BATCH, location = cow_dex::settlement)]
fun test_process_intent_wrong_batch_id_aborts() {
    let mut ctx = tx_context::dummy();

    let mut ticket = settlement::create_ticket_for_testing(42, 1);
    let intent = intent_book::create_intent_for_testing<SUI, USDC>(100, 50, 9999, 99, &mut ctx);
    let payout = coin::mint_for_testing<USDC>(60, &mut ctx);
...
fun test_full_settlement_flow() {
    ...
    ts::next_tx(&mut scenario, winner);
    {
        ...
        clock::set_for_testing(&mut clock, 3000);
        let mut ticket = settlement::open_settlement(&mut state, &clock, ctx);
        ...
        let payout = coin::mint_for_testing<USDC>(60, ctx);
        settlement::process_intent<SUI, USDC>(&mut ticket, intent, payout, 50, 55, ctx);
    ...
    ts::next_tx(&mut scenario, winner);
    {
        let bond_refund = ts::take_from_sender<coin::Coin<SUI>>(&scenario);
        assert!(coin::value(&bond_refund) == 1_000_000_000);
        ts::return_to_sender(&scenario, bond_refund);
    };
